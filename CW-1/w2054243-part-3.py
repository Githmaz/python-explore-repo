...
#______ Write a result to the file and displays it _____#

def write_and_show_results(result,marks):

    with open('result_data.txt', 'a') as file:
            file.write(f"{result} - {marks[0]}, {marks[1]}, {marks[2]}" + '\n')
    ...
    while True:
        try:
            ...
            if pass_credits + defer_credits + fail_credits == 120:
                ...
                if pass_credits == 120:
                    write_and_show_results("Progress",[pass_credits,defer_credits,fail_credits])
                elif pass_credits == 100:
                    write_and_show_results("Progress (Module Trailer)",[pass_credits,defer_credits,fail_credits])
                elif fail_credits >= 80:
                    write_and_show_results(f"Exclude",[pass_credits,defer_credits,fail_credits])
                else :
                    write_and_show_results(f"Module Retriever",[pass_credits,defer_credits,fail_credits])
            else:
                print("\nTotal Incorrect")
                continue
